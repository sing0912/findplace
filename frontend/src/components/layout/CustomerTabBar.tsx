/**
 * @fileoverview 반려인 하단 탭 네비게이션
 */

import React from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { BottomNavigation, BottomNavigationAction, Paper } from '@mui/material';
import { Home, Search, CalendarMonth, ChatBubble, Person } from '@mui/icons-material';

const tabs = [
  { label: '홈', icon: <Home />, path: '/' },
  { label: '검색', icon: <Search />, path: '/search' },
  { label: '예약', icon: <CalendarMonth />, path: '/bookings' },
  { label: '채팅', icon: <ChatBubble />, path: '/chat' },
  { label: '마이', icon: <Person />, path: '/my' },
];

const CustomerTabBar: React.FC = () => {
