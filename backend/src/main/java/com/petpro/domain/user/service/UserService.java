...
public class UserService {
     ...
     * @return 수정된 사용자 정보
     * @throws BusinessException 닉네임 중복 시
     */
    @Transactional
    public UserResponse.Info updateNickname(Long userId, UserRequest.UpdateNickname request) {
     ...
     * @param userId 사용자 ID
     * @param request 비밀번호 변경 요청 DTO
     * @throws BusinessException 현재 비밀번호 불일치, 소셜 로그인 사용자, 비밀번호 강도 미달 시
     */
    @Transactional
    public void changeMyPassword(Long userId, UserRequest.ChangeMyPassword request) {
        User user = findUserById(userId);
     ...
     * @throws BusinessException 비밀번호 불일치 시 (일반 사용자)
     */
    @Transactional
    public void deleteMyAccount(Long userId, UserRequest.DeleteAccount request) {
        User user = findUserById(userId);
