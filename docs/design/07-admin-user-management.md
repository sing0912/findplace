# 관리자 회원 관리 설계

## 1. 개요

관리자 회원 관리 기능은 시스템 관리자가 회원 정보를 조회, 검색, 수정, 상태 변경할 수 있는 기능을 제공합니다. 권한 수준에 따라 접근 가능한 기능이 제한됩니다.

---

## 2. 권한 체계

### 2.1 역할별 권한

| 기능 | USER | COMPANY_ADMIN | SUPPLIER_ADMIN | ADMIN | SUPER_ADMIN |
|------|------|---------------|----------------|-------|-------------|
| 회원 목록 조회 | ❌ | ❌ | ❌ | ✅ | ✅ |
| 회원 상세 조회 | ❌ | ❌ | ❌ | ✅ | ✅ |
| 회원 정보 수정 | ❌ | ❌ | ❌ | ✅ | ✅ |
| 회원 상태 변경 | ❌ | ❌ | ❌ | ✅ | ✅ |
| 회원 역할 변경 | ❌ | ❌ | ❌ | ❌ | ✅ |
| 회원 삭제 | ❌ | ❌ | ❌ | ✅ | ✅ |
| 관리자 계정 생성 | ❌ | ❌ | ❌ | ❌ | ✅ |

### 2.2 상태 변경 가능 범위

| 현재 상태 | 변경 가능 상태 | 권한 |
|----------|---------------|------|
| ACTIVE | INACTIVE, SUSPENDED | ADMIN |
| INACTIVE | ACTIVE, SUSPENDED | ADMIN |
| SUSPENDED | ACTIVE, INACTIVE | ADMIN |
| DELETED | 복구 불가 | - |

---

## 3. API 설계

### 3.1 회원 관리 API

| Method | Endpoint | 설명 | 권한 |
|--------|----------|------|------|
| GET | /admin/users | 회원 목록 조회 | ADMIN |
| GET | /admin/users/{id} | 회원 상세 조회 | ADMIN |
| GET | /admin/users/search | 회원 검색 | ADMIN |
| PUT | /admin/users/{id} | 회원 정보 수정 | ADMIN |
| PATCH | /admin/users/{id}/status | 상태 변경 | ADMIN |
| PATCH | /admin/users/{id}/role | 역할 변경 | SUPER_ADMIN |
| DELETE | /admin/users/{id} | 회원 삭제 | ADMIN |
| POST | /admin/users | 관리자 계정 생성 | SUPER_ADMIN |

### 3.2 회원 연관 데이터 API

| Method | Endpoint | 설명 | 권한 |
|--------|----------|------|------|
| GET | /admin/users/{id}/pets | 회원의 반려동물 목록 | ADMIN |
| GET | /admin/users/{id}/coupons | 회원의 쿠폰 목록 | ADMIN |
| GET | /admin/users/{id}/orders | 회원의 주문 내역 | ADMIN |
| GET | /admin/users/{id}/reservations | 회원의 예약 내역 | ADMIN |
| GET | /admin/users/{id}/activity-logs | 회원 활동 로그 | ADMIN |

---

## 4. 요청/응답 DTO

### 4.1 회원 목록 조회 요청

```
GET /admin/users?page=0&size=20&sort=createdAt,desc&status=ACTIVE&role=USER&keyword=홍길동
```

| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| page | Integer | X | 페이지 번호 (기본: 0) |
| size | Integer | X | 페이지 크기 (기본: 20) |
| sort | String | X | 정렬 (기본: createdAt,desc) |
| status | String | X | 상태 필터 |
| role | String | X | 역할 필터 |
| keyword | String | X | 검색어 (이름, 이메일, 전화번호) |
| startDate | LocalDate | X | 가입일 시작 |
| endDate | LocalDate | X | 가입일 종료 |

### 4.2 회원 목록 응답

```json
{
  "content": [
    {
      "id": 1,
      "email": "user@example.com",
      "name": "홍길동",
      "phone": "010-1234-5678",
      "role": "USER",
      "roleName": "일반 사용자",
      "status": "ACTIVE",
      "statusName": "활성",
      "petCount": 2,
      "couponCount": 5,
      "lastLoginAt": "2025-01-25T09:30:00",
      "createdAt": "2025-01-20T14:30:00"
    }
  ],
  "totalElements": 150,
  "totalPages": 8,
  "number": 0,
  "size": 20
}
```

### 4.3 회원 상세 응답

```json
{
  "id": 1,
  "email": "user@example.com",
  "name": "홍길동",
  "phone": "010-1234-5678",
  "birthDate": "1990-05-15",
  "address": "서울특별시 강남구 테헤란로 123",
  "addressDetail": "101동 202호",
  "zipCode": "06141",
  "latitude": 37.5065,
  "longitude": 127.0536,
  "role": "USER",
  "roleName": "일반 사용자",
  "status": "ACTIVE",
  "statusName": "활성",
  "profileImageUrl": "https://storage.petpro.com/profiles/1.jpg",
  "lastLoginAt": "2025-01-25T09:30:00",
  "createdAt": "2025-01-20T14:30:00",
  "updatedAt": "2025-01-25T10:00:00",
  "summary": {
    "petCount": 2,
    "alivePetCount": 1,
    "deceasedPetCount": 1,
    "activeCouponCount": 5,
    "usedCouponCount": 3,
    "totalOrderCount": 10,
    "totalOrderAmount": 1500000,
    "reservationCount": 2
  }
}
```

### 4.4 상태 변경 요청

```json
{
  "status": "SUSPENDED",
  "reason": "비정상적인 활동 감지"
}
```

### 4.5 역할 변경 요청

```json
{
  "role": "COMPANY_ADMIN",
  "reason": "업체 관리자 권한 부여"
}
```

### 4.6 회원 정보 수정 요청

```json
{
  "name": "홍길동",
  "phone": "010-9876-5432",
  "birthDate": "1990-05-15",
  "address": "서울특별시 서초구 서초대로 456",
  "addressDetail": "201동 301호",
  "zipCode": "06500"
}
```

---

## 5. 비즈니스 로직

### 5.1 회원 검색 로직

```java
@Service
public class AdminUserService {

    public Page<UserListDto> searchUsers(UserSearchRequest request, Pageable pageable) {
        Specification<User> spec = Specification.where(null);

        if (StringUtils.hasText(request.getKeyword())) {
            spec = spec.and((root, query, cb) ->
                cb.or(
                    cb.like(root.get("name"), "%" + request.getKeyword() + "%"),
                    cb.like(root.get("email"), "%" + request.getKeyword() + "%"),
                    cb.like(root.get("phone"), "%" + request.getKeyword() + "%")
                )
            );
        }

        if (request.getStatus() != null) {
            spec = spec.and((root, query, cb) ->
                cb.equal(root.get("status"), request.getStatus())
            );
        }

        if (request.getRole() != null) {
            spec = spec.and((root, query, cb) ->
                cb.equal(root.get("role"), request.getRole())
            );
        }

        // Soft Delete 제외
        spec = spec.and((root, query, cb) -> cb.isNull(root.get("deletedAt")));

        return userRepository.findAll(spec, pageable)
            .map(this::toListDto);
    }
}
```

### 5.2 상태 변경 로직

```java
@Transactional
public void changeStatus(Long userId, StatusChangeRequest request, Long adminId) {
    User user = userRepository.findById(userId)
        .orElseThrow(() -> new EntityNotFoundException("User not found"));

    // 상태 변경 유효성 검사
    validateStatusChange(user.getStatus(), request.getStatus());

    user.setStatus(request.getStatus());

    // 변경 이력 저장
    UserStatusChangeLog log = new UserStatusChangeLog();
    log.setUserId(userId);
    log.setFromStatus(user.getStatus());
    log.setToStatus(request.getStatus());
    log.setReason(request.getReason());
    log.setChangedBy(adminId);
    log.setChangedAt(LocalDateTime.now());
    statusChangeLogRepository.save(log);

    // 정지 시 알림 발송
    if (request.getStatus() == UserStatus.SUSPENDED) {
        notificationService.sendSuspensionNotice(user, request.getReason());
    }
}
```

### 5.3 역할 변경 로직

```java
@Transactional
@PreAuthorize("hasRole('SUPER_ADMIN')")
public void changeRole(Long userId, RoleChangeRequest request, Long adminId) {
    User user = userRepository.findById(userId)
        .orElseThrow(() -> new EntityNotFoundException("User not found"));

    UserRole oldRole = user.getRole();
    user.setRole(request.getRole());

    // 변경 이력 저장
    UserRoleChangeLog log = new UserRoleChangeLog();
    log.setUserId(userId);
    log.setFromRole(oldRole);
    log.setToRole(request.getRole());
    log.setReason(request.getReason());
    log.setChangedBy(adminId);
    log.setChangedAt(LocalDateTime.now());
    roleChangeLogRepository.save(log);

    // 역할 변경 알림
    notificationService.sendRoleChangeNotice(user, oldRole, request.getRole());
}
```

### 5.4 회원 삭제 (Soft Delete)

```java
@Transactional
public void deleteUser(Long userId, Long adminId) {
    User user = userRepository.findById(userId)
        .orElseThrow(() -> new EntityNotFoundException("User not found"));

    // SUPER_ADMIN은 삭제 불가
    if (user.getRole() == UserRole.SUPER_ADMIN) {
        throw new BusinessException("최고 관리자 계정은 삭제할 수 없습니다.");
    }

    // Soft Delete
    user.setDeletedAt(LocalDateTime.now());
    user.setDeletedBy(adminId);
    user.setStatus(UserStatus.DELETED);

    // 개인정보 마스킹
    user.setEmail(maskEmail(user.getEmail()));
    user.setName("탈퇴회원");
    user.setPhone(null);

    // 삭제 이벤트 발행
    eventPublisher.publishEvent(new UserDeleteEvent(userId, adminId));
}
```

---

## 6. 프론트엔드 UI

### 6.1 회원 목록 페이지

```
┌─────────────────────────────────────────────────────────────────────────┐
│  회원 관리                                                    [+ 회원 생성] │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  검색: [                    🔍]                                          │
│                                                                          │
│  상태: [전체 ▼]  역할: [전체 ▼]  가입일: [시작일] ~ [종료일]  [검색] [초기화]│
│                                                                          │
├─────────────────────────────────────────────────────────────────────────┤
│  ☐ │ 이메일              │ 이름    │ 역할      │ 상태  │ 최근 로그인      │
│────────────────────────────────────────────────────────────────────────── │
│  ☐ │ user1@example.com  │ 홍길동  │ 일반 사용자│ 활성  │ 2025-01-25 09:30│
│  ☐ │ admin@example.com  │ 김관리  │ 관리자    │ 활성  │ 2025-01-25 10:00│
│  ☐ │ user2@example.com  │ 이영희  │ 일반 사용자│ 정지  │ 2025-01-20 15:20│
│────────────────────────────────────────────────────────────────────────── │
│  선택: 0명                           < 1 2 3 4 5 ... 8 > 20개씩 ▼         │
│                                                                          │
│  [상태 변경] [삭제]                                                       │
└─────────────────────────────────────────────────────────────────────────┘
```

### 6.2 회원 상세 페이지

```
┌─────────────────────────────────────────────────────────────────────────┐
│  ← 회원 관리                                            [수정] [삭제]    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌────────┐                                                              │
│  │ 프로필  │  홍길동                                                      │
│  │ 이미지  │  user@example.com                                           │
│  └────────┘  📞 010-1234-5678                                            │
│              🎂 1990-05-15 (35세)                                         │
│              📍 서울특별시 강남구 테헤란로 123                              │
│                                                                          │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │ 역할: 일반 사용자 [변경]     상태: 🟢 활성 [변경]                     │ │
│  │ 가입일: 2025-01-20          최근 로그인: 2025-01-25 09:30           │ │
│  └────────────────────────────────────────────────────────────────────┘ │
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ 반려동물 (2)│ 쿠폰 (5) │ 주문 (10) │ 예약 (2) │ 활동 로그          │   │
│  ├─────────────────────────────────────────────────────────────────┤   │
│  │                                                                  │   │
│  │  ┌──────────────┐ ┌──────────────┐                              │   │
│  │  │ 🐕 콩이      │ │ 🐕 보리 🌈   │                              │   │
│  │  │ 말티즈 · 수컷│ │ 골든리트리버 │                              │   │
│  │  │ 4세         │ │ 추모 중      │                              │   │
│  │  └──────────────┘ └──────────────┘                              │   │
│  │                                                                  │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### 6.3 상태 변경 다이얼로그

```
┌─────────────────────────────────────────┐
│  회원 상태 변경                      [X] │
├─────────────────────────────────────────┤
│                                         │
│  대상 회원: 홍길동 (user@example.com)    │
│  현재 상태: 🟢 활성                      │
│                                         │
│  변경할 상태:                           │
│  ○ 비활성 - 로그인 가능, 기능 제한      │
│  ● 정지   - 로그인 불가                 │
│                                         │
│  사유 *                                 │
│  ┌───────────────────────────────────┐ │
│  │ 비정상적인 활동 감지               │ │
│  │                                   │ │
│  └───────────────────────────────────┘ │
│                                         │
│  ⚠️ 정지 처리 시 회원에게 알림이 발송됩니다.│
│                                         │
│              [취소]  [변경]              │
└─────────────────────────────────────────┘
```

### 6.4 역할 변경 다이얼로그 (SUPER_ADMIN 전용)

```
┌─────────────────────────────────────────┐
│  회원 역할 변경                      [X] │
├─────────────────────────────────────────┤
│                                         │
│  대상 회원: 김관리 (admin@example.com)   │
│  현재 역할: 관리자 (ADMIN)               │
│                                         │
│  변경할 역할:                           │
│  ○ 일반 사용자 (USER)                   │
│  ○ 업체 관리자 (COMPANY_ADMIN)          │
│  ○ 공급사 관리자 (SUPPLIER_ADMIN)       │
│  ● 관리자 (ADMIN)                       │
│  ○ 최고 관리자 (SUPER_ADMIN)            │
│                                         │
│  사유 *                                 │
│  ┌───────────────────────────────────┐ │
│  │ 업체 관리자 권한 부여              │ │
│  └───────────────────────────────────┘ │
│                                         │
│  ⚠️ 역할 변경은 즉시 적용됩니다.          │
│                                         │
│              [취소]  [변경]              │
└─────────────────────────────────────────┘
```

---

## 7. 변경 이력 관리

### 7.1 UserStatusChangeLog 엔티티

```
┌─────────────────────────────────────────────────────────────┐
│                   UserStatusChangeLog                       │
├─────────────────────────────────────────────────────────────┤
│  id                  BIGINT PK AUTO_INCREMENT               │
│  userId              BIGINT NOT NULL                        │
│  fromStatus          VARCHAR(20) NOT NULL                   │
│  toStatus            VARCHAR(20) NOT NULL                   │
│  reason              TEXT                                   │
│  changedBy           BIGINT NOT NULL                        │
│  changedAt           TIMESTAMP NOT NULL                     │
└─────────────────────────────────────────────────────────────┘
```

### 7.2 UserRoleChangeLog 엔티티

```
┌─────────────────────────────────────────────────────────────┐
│                    UserRoleChangeLog                        │
├─────────────────────────────────────────────────────────────┤
│  id                  BIGINT PK AUTO_INCREMENT               │
│  userId              BIGINT NOT NULL                        │
│  fromRole            VARCHAR(20) NOT NULL                   │
│  toRole              VARCHAR(20) NOT NULL                   │
│  reason              TEXT                                   │
│  changedBy           BIGINT NOT NULL                        │
│  changedAt           TIMESTAMP NOT NULL                     │
└─────────────────────────────────────────────────────────────┘
```

---

## 8. 인덱스

```sql
-- 회원 검색용
CREATE INDEX idx_users_name ON users(name);
CREATE INDEX idx_users_phone ON users(phone);
CREATE INDEX idx_users_created_at ON users(created_at DESC);

-- 변경 이력 조회용
CREATE INDEX idx_status_log_user ON user_status_change_logs(user_id);
CREATE INDEX idx_status_log_changed_at ON user_status_change_logs(changed_at DESC);
CREATE INDEX idx_role_log_user ON user_role_change_logs(user_id);
CREATE INDEX idx_role_log_changed_at ON user_role_change_logs(changed_at DESC);
```

---

## 9. 보안 설정

### 9.1 Spring Security 설정

```java
@Configuration
@EnableMethodSecurity
public class SecurityConfig {

    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/admin/**").hasAnyRole("ADMIN", "SUPER_ADMIN")
                .requestMatchers("/admin/users/*/role").hasRole("SUPER_ADMIN")
                .requestMatchers(HttpMethod.POST, "/admin/users").hasRole("SUPER_ADMIN")
                .anyRequest().authenticated()
            )
            .build();
    }
}
```

### 9.2 메소드 보안

```java
@Service
public class AdminUserService {

    @PreAuthorize("hasAnyRole('ADMIN', 'SUPER_ADMIN')")
    public Page<UserListDto> searchUsers(...) { }

    @PreAuthorize("hasAnyRole('ADMIN', 'SUPER_ADMIN')")
    public UserDetailDto getUserDetail(Long id) { }

    @PreAuthorize("hasRole('SUPER_ADMIN')")
    public void changeRole(Long userId, RoleChangeRequest request) { }

    @PreAuthorize("hasRole('SUPER_ADMIN')")
    public void createAdmin(AdminCreateRequest request) { }
}
```

---

## 10. 감사 로그

### 10.1 관리자 활동 로그

모든 관리자 작업은 감사 로그로 기록됩니다.

```java
@Aspect
@Component
public class AdminAuditAspect {

    @AfterReturning("@annotation(AdminAudit)")
    public void logAdminAction(JoinPoint joinPoint) {
        String action = getActionName(joinPoint);
        String targetId = getTargetId(joinPoint);
        Long adminId = SecurityUtils.getCurrentUserId();

        AdminAuditLog log = new AdminAuditLog();
        log.setAdminId(adminId);
        log.setAction(action);
        log.setTargetType("USER");
        log.setTargetId(targetId);
        log.setIpAddress(RequestUtils.getClientIp());
        log.setCreatedAt(LocalDateTime.now());

        auditLogRepository.save(log);
    }
}
```

### 10.2 감사 로그 조회 UI

```
┌─────────────────────────────────────────────────────────────────────────┐
│  활동 로그 - 홍길동 (user@example.com)                                   │
├─────────────────────────────────────────────────────────────────────────┤
│  일시                │ 작업자     │ 작업           │ 상세               │
│──────────────────────────────────────────────────────────────────────── │
│  2025-01-25 10:30   │ 김관리    │ 상태 변경      │ 활성 → 정지        │
│  2025-01-24 15:20   │ 박관리    │ 정보 수정      │ 전화번호 변경      │
│  2025-01-20 14:30   │ 시스템    │ 계정 생성      │ 회원가입           │
└─────────────────────────────────────────────────────────────────────────┘
```

